#!/usr/bin/env python3

from __future__ import annotations

import argparse

import pandas as pd

try:
    from pyscripts.common import calculate_wilder_atr_series, load_stock_data, validate_day_counts
except ModuleNotFoundError:
    from common import calculate_wilder_atr_series, load_stock_data, validate_day_counts


def _calculate_wilder_atr(df: pd.DataFrame, day_count: int) -> float:
    atr_series = calculate_wilder_atr_series(df, day_count)
    return float(atr_series.dropna().iloc[-1])


def return_atr(symbol: str, day_counts: list[int], as_of_date: str) -> dict[int, float]:
    validated_day_counts = validate_day_counts(day_counts)
    normalized_symbol, df, resolved_date = load_stock_data(symbol, as_of_date, ["high", "low", "close"])

    atr_values: dict[int, float] = {}
    usable_rows = len(df)
    for day_count in validated_day_counts:
        if usable_rows <= day_count:
            raise ValueError(
                f"Not enough data to compute ATR({day_count}) for {normalized_symbol} "
                f"through {resolved_date.date().isoformat()}"
            )
        atr_values[day_count] = _calculate_wilder_atr(df, day_count)

    return atr_values


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Return latest ATR values for a stock symbol")
    parser.add_argument("symbol", help="Stock symbol from stocks.txt")
    parser.add_argument("day_counts", nargs="+", type=int, help="ATR day counts to calculate")
    parser.add_argument("--date", required=True, dest="as_of_date", help="As-of date in YYYY-MM-DD format")
    return parser


def main() -> None:
    parser = _build_parser()
    args = parser.parse_args()
    print(return_atr(args.symbol, args.day_counts, args.as_of_date))


if __name__ == "__main__":
    main()
